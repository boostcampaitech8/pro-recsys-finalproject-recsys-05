...
class RankingDatasetBuilder:
    def __init__(self):
        ...
        logger.info("=" * 80)
        logger.info("Task 1: Ranking Dataset Builder 시작")
        logger.info("=" * 80)
    ...
    def merge_candidates(self, ease_candidates, lightgcn_candidates):
        ...
        for user_id in ease_candidates.keys():
            ...
            # EASE 후보
            ease_items = {str(item['item_id']): item['score'] for item in ease_candidates[user_id]}

            # LightGCN 후보
            lightgcn_items = {str(item['item_id']): item['score'] for item in lightgcn_candidates[user_id]}
    ...
    def create_samples(self, merged_candidates, item_df, user_df):
        ...
        logger.info(f"  - Positive: {positive_count:,}개 ({100*positive_count/len(all_samples):.1f}%)")
        logger.info(f"  - Negative: {negative_count:,}개 ({100*negative_count/len(all_samples):.1f}%)")
