...
#### T30 · dev push 배포 빌드 검증 자동화 + 로컬 pre-push  [ci-cd] [code] [low] [doing]  → **PR #132 (feature→dev)** · 2026-07-13
- 문제: feature→dev를 PR로 올리면 `compose_pr`(배포 이미지 빌드 회귀)가 돌지만 **dev 직접 push는 우회**(compose_pr가 `pull_request` 전용) → 스테이징 배포 검증 구멍.
- 반영: `deploy.yml` compose_pr를 `PR + dev push`로 확장(main은 build_push arm64 실빌드가 커버) · `scripts/predeploy_check.sh`(compose config + unit, `PREDEPLOY_BUILD=1` 시 이미지 빌드) · `scripts/hooks/pre-push`(dev/main push 자동 — `git config core.hooksPath scripts/hooks` 활성화) · `.gitattributes`(셸/훅 LF 강제).
