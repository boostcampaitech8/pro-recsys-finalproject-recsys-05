...
## step3 · hard failure gate 행위변경

**[실행 요약]**
- 커밋: 하네스 담당(코드 레인 하위 실행자는 commit 미수행).
- 내용: CLI가 runner의 종료 코드를 실제 프로세스 종료 코드로 전달하도록 바꾸고 halted·push·branch·`gh pr create`·Codex 재시도 소진·task/step 파일 오류를 non-zero로 통일했다. `--from`은 정확히 일치하는 step이 없으면 사용 목록과 함께 exit 2로 종료한다. 빈 verify는 Codex 실행 전에 실패하며 `verify: skip`만 명시적 스킵으로 허용하고, verify timeout은 traceback 대신 실패 결과로 변환한다.
- 테스트: 명령별 rc/stdout/예외를 주입하는 fake subprocess fixture와 gates/CLI unit 테스트를 추가했다. 빈 verify 전용 invalid fixture, 누락 step fixture를 두고 push·branch·gh·Codex rc/timeout·halted 경로를 외부 의존 없이 검증한다.
- 골든 변경: task_sample step2를 `empty verify fixture`에서 `explicit verify skip fixture`로 바꾸고 본문과 `verify: skip` 출력 한 줄만 의도적으로 갱신했다. T4 step1도 같은 명시 계약으로 변경했다.
- 검증: 쓰기 가능한 임시 uv 캐시로 `cd backend && uv run pytest ../scripts/exec_harness/tests -q` → 14 passed(기본 pytest cache 경로는 관리형 샌드박스 접근 경고 1건). `python scripts/execute.py --task scripts/exec_harness/tests/fixtures/task_sample --dry-run` → exit 0.
- 다음 step에 주는 것: T23 코드 변경은 완료됐으며, 하네스가 최종 verify·diff·커밋·PR·교차리뷰를 수행한다. manifest/handoff·resume·review 상태기계·모델 라우팅·worktree는 후속 T24~T29 경계로 남겼다.
