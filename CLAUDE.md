...
### Phase 0 — 데이터 확보 (블로커, 진행 중)
- [ ] **(사용자)** GCP 결제계정 재연결 → 백업 스크립트 재실행 (스크립트 사양: PLAN.md §2-①)
- [ ] GCS 전체 백업 → 로컬 + `G:\내 드라이브\부스트캠프\backup` 이중 보관
- [ ] (병행) 메인컴퓨터/팀원 로컬에서 아티팩트 수색 (목록: PLAN.md §2-②)
- [ ] (병행) `postgres_data` 볼륨에서 games 테이블 pg_dump 복구 시도
