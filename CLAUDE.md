...
Steam 게임 추천시스템(FastAPI + pgvector + Redis + BentoML + React, 부스트캠프 2026년 1~2월 프로젝트). **reactivation Phase 0~4 완료 → 현재는 유지보수 모드다.** 유지보수의 단일 진실 소스는 **`docs/MAINTENANCE.md`**(컴포넌트 지도·seam registry·티켓 백로그·진행 프로토콜). 아래 헌법은 매 세션이 지켜야 할 불변식과 진입점만 담고, reactivation 진행 로그는 이 파일 하단에 기록용으로 보존한다.

## 유지보수 헌법 (Constitution)

**불변식 (절대 어기지 말 것)**
1. prod 추천은 **EASE 폴백 유지가 기본** — BentoML 실추천은 PR #83로 영구화 진행 중 (상세: MAINTENANCE T5 · seam S3).
2. `ml_rec/scripts/stage4_serving/model_loader.py`의 **후보 JSON 로드 스킵을 되돌리지 말 것** (12GB OOM · seam S2).
3. **임베딩은 bge-m3(1024차원) 교체 금지** (seam S6).
4. 배포 이미지는 ARM — **buildx arm64 필수**.
5. **컴포넌트 경계(seam) 변경은 한 커밋으로** (예: `nginx.conf`+`compose`+`deploy.yml` · MAINTENANCE §2).
6. 데이터/모델 아티팩트(`*.pkl`·`*.inter`·`*.jsonl` 등) **커밋 금지**.

**컴포넌트 지도(요약)**: frontend(Vercel) · backend(FastAPI; 도메인 chat〔orchestration〕/recommendation/game/steam/user) · ai-recsys(`ml_rec/` BentoML 3-stage) · ci-cd·infra(deploy.yml·compose·nginx·Oracle A1). 상세 = MAINTENANCE §1.

**진입점 (admission gate · ADR-0004)**: 유지보수 세션은 → **`docs/MAINTENANCE.md`** §0 ritual을 따른다. **non-trivial 작업(코드·인프라·prod 변경 / seam 접촉 / 2개+ 컴포넌트 횡단)은 실행 전 §0 intake를 반드시 통과**한다 — 조사→논의→**step 배치**→티켓(MAINTENANCE §3 + Issue 동시 생성). 읽기전용 조사·자명한 단일수정만 인라인 허용(로그). 세션 종료 시 **sweep**(건드린 Issue ↔ §3 정의 1:1)로 정합 확인. 브랜치는 `feature → dev → main`(main 직행 금지, dev 스테이징).

**문서 역할(ADR-0002)**: 설계 결정=`docs/adr/`, 제품 방향=`docs/PRD.md`, 유지보수 구조·티켓=`docs/MAINTENANCE.md`, **라이브 진행상태=GitHub Issues**.

## 작업 규칙

- 계획/조사 문서: 유지보수 = `docs/MAINTENANCE.md`(정본), reactivation 기록 = `docs/reactivation/` (`PLAN.md`·`HANDOFF.md`). `revive/reactivation` 브랜치는 reactivation 전용(완료).
- 루트 `README.md`의 프로젝트 구조도는 **구식이라 신뢰 금지** — 실제 트리는 `ml_rec/scripts/{preprocessing,stage1_retrieval,stage2_ranking,stage3_scoring,stage4_serving}` 기준 (MAINTENANCE T4).
- `ml_rec/scripts/stage4_serving/model_loader.py`의 후보 JSON 로드 스킵을 되돌리지 말 것 (12GB 서버에서 OOM — 헌법 불변식 2 · seam S2).
- 데이터/모델 아티팩트(`*.pkl`, `*.inter`, `*.jsonl` 등)는 gitignore 대상 — 커밋 금지 (헌법 불변식 6).
...
## (기록용) reactivation 진행 로그 — Phase 0~4 완료

> 아래는 reactivation(2026-07-06~07) 진행 기록이며 기록용으로 보존한다. 현행 운영·잔여 작업은 `docs/MAINTENANCE.md` 참조.
...
- [x] **BentoML 3-stage 영구화 (2026-07-07)**: 노트북 로컬 실추천 검증 통과(근본원인=EASE torch체크포인트 pickle불가 → 신규유저 후보 생성 복구, ml_rec 2파일 수정) 후 **재배포에도 유지되도록 정식 통합**. ① prod.yml에 bentoml 서비스 추가(mem 4G, `BENTOML_SKIP_GCS_BOOTSTRAP=true`, ml_rec bind-mount) ② CI `build_push`에 arm64 `rec-bentoml` 빌드/푸시 스텝 추가 ③ deploy.sh preflight에 아티팩트 4종(EASE backend-format 1.27GB·dcn·xgb·lightgcn) 상주 하드체크(없으면 abort, `ALLOW_MISSING_BENTOML=1`로 EASE 폴백 배포). 아티팩트는 서버 고정경로 bind-mount(이미지 미포함). 런북·재현: `docs/reactivation/BENTOML_VERIFY.md`, 배경: HANDOFF.md §0. ⚠️ 첫 재배포 전 서버 dirty checkout 정리 필요(HANDOFF §0-B1)
