...
### Phase 4 — 프론트 Vercel 이전 ← 진행 중 (2026-07-07)
...
- [ ] 사용자 HITL: **`VERCEL_TOKEN`만 남음** — CLI 세션으로는 API 토큰 발급 금지("Cannot create tokens for this app")라 대시보드(Account Settings→Tokens)에서 발급 후 `gh secret set VERCEL_TOKEN -R boostcampaitech8/pro-recsys-finalproject-recsys-05`로 직접 등록 (값이 채팅/로그에 안 남는 경로). 미등록 상태로 main 머지되면 프론트 잡만 명확한 에러로 실패(백엔드 배포는 무관)
